...
export default function Checkout() {
    ...
    const { carrito, sucursalId, setSucursalId, sucursales, total, quitarDelCarrito, actualizarCantidad, vaciarCarrito } = useCarrito()
                            ...
                            {!sucursalId && (
                                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6">
                                    <p className="text-yellow-800 font-medium mb-3">
                                        ⚠️ Selecciona una sucursal para continuar
                                    </p>
                                    <div className="flex flex-wrap gap-2">
                                        {sucursales.map(suc => (
                                            <button
                                                key={suc.id}
                                                className="px-4 py-2 rounded-lg border border-yellow-300 bg-white hover:bg-yellow-100 transition text-sm font-medium text-yellow-800"
                                                onClick={() => setSucursalId(suc.id)}
                                            >
                                                {suc.moneda === 'USD' ? '🇪🇨' : '🇨🇴'} {suc.nombre}
                                            </button>
                                        ))}
                                    </div>
                                </div>
